<template>
  <v-row tag="section" class="inventory">
    <v-col v-for="inventory in inventories" :key="inventory.id" cols="12">
      <v-card @click="toMaterialItem(inventory.materialId)">
        <v-container>
          <v-row>
            <v-col
              v-text="getItem(materials, inventory.materialId).controlCode"
              tag="span"
              cols="12"
              class="body-1 py-0 my-1"
            />
            <v-col
              v-text="getItem(materials, inventory.materialId).name"
              tag="h1"
              cols="12"
              class="headline font-weight-bold py-0 my-1"
            />
            <v-col
              v-text="`(製造番号: ${inventory.lotCode})`"
              tag="h2"
              cols="12"
              class="body-1 py-0 my-1"
            />
