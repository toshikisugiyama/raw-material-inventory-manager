<template lang="pug">
  v-container.pa-0
    ...
    v-row.material(v-if="!!currentInventory" tag="section")
      v-col(v-text="currentInventory.lotCode" tag="h1" cols="12" class="headline font-weight-bold py-0 my-1")
      v-col(cols="12")
        v-form
          v-row
            v-col(cols="12")
              v-text-field(v-model="usedAmount" :append-outer-icon="currentMaterial.unit" type="number" label="使用量" required autofocus outlined)
          v-row
            v-col(cols="12")
              v-text-field(:value="stockAmount" :append-outer-icon="currentMaterial.unit" type="number" label="在庫" readonly required outlined)
          v-btn(@click="submit" :disabled="!usedAmount || stockAmount < 0" depressed large) 決定
</template>
